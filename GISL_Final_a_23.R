...
## Input datasets: MuTect SSMs, Battenberg CNVs and Battenberg cellularity estimate
...
if(length(which(optima.0 >= 0.1)) == 1 && sum(copyNumber.ssm[,"subclonal"]) == 0) {
  ...
  optima = max(optima.0, cellularity0)
  cellularity = optima
  no.clusters = 1
  ...
  # Use the largest estimate as the answer for cellularity
  cellularity <- max(max(optima.1), max(optima.0), cellularity0)
  no.clusters <- length(optima.1)
  optima <- c(cellularity, optima.1[2:no.clusters])
  
  ## Get number of SSMs in each subpopulation using Gausian mixture model
  # library("mixtools")
  new_coef1 <- cellularity*copyNumber.ssm[,"tumor_cn"] + (1 - cellularity)*copyNumber.ssm[,"normal_cn"]
  set.seed(123456)
  tmp1 <- normalmixEM((as.numeric(tumor_stat[,"FA"])*new_coef1)[setdiff(1:nrow(ssmdat),id1.to.remove)], mu = c(cellularity*2, optima), mean.constr = c(cellularity*2, optima))

  no.mut.cluster <- rep(0, no.clusters+1)
  no.mut.cluster[no.clusters+1] <- num.FP
  for(i in 2:no.clusters){
    no.mut.cluster[i] <- round(tmp1$lambda[i+1]*length(tmp1$x))
  }
  no.mut.cluster[no.clusters] <- max(0, no.mut.cluster[no.clusters] + length(id1.to.remove) - num.FP)
  no.mut.cluster[1] <- no.muts - sum(no.mut.cluster)
  ...
  ## Assignment method 1: use posterior matrix from gausian mixture model
  
  posterior.matrix <- tmp1$posterior
  posterior.matrix[,1] <- posterior.matrix[,1] + posterior.matrix[,2]
  posterior.matrix <- posterior.matrix[,-2]
  probs <- matrix(0, nrow = no.muts, ncol = no.clusters)
  
  if(!is.na(sum(posterior.matrix))) {
    probs[setdiff(1:no.muts,id1.to.remove),] <- posterior.matrix
    probs[id1.to.remove, no.clusters] <- 1
    rm(posterior.matrix); gc(verbose = FALSE)
  } else {
    # In case of NA in posterior output of function normalmixEM(), use binomial probability as in method B.
    new_coef1 <- cellularity*copyNumber.ssm[,"tumor_cn"] + (1 - cellularity)*copyNumber.ssm[,"normal_cn"]
    for(i in 1:no.muts) {
  ...
  # Determine mutation assignment for SC_2A
  assignments <- apply(probs,1, which.max)
  if(length(unique(assignments)) < no.clusters) {
    assignments[setdiff(1:no.muts,id1.to.remove)] <- match(cluster.assignment$most.likely.cluster,occupied.clusters)
    assignments[id1.to.remove] <- no.clusters
  }
  ...
  # system("gzip GISL_subchallenge2B.txt")
  ...
  for(i in 1:(no.muts-1)) {
    for(j in (i+1):no.muts) {
      ...
      ADM[i,j] <- min(round(p1, digits = 4),1)
  ...
  # system("gzip GISL_subchallenge3B.txt")
